	...
	class DB_banhang
{
    ...
    function connect()
    {
        ...
        if (!$this->__conn){
            // Kết nối
            $this->__conn = mysqli_connect('localhost', 'root', '', 'bhol') or die ('Lỗi kết nối');
    ...
    // Hàm Update
    function update($table, $data, $colunm, $where){
        // Kết nối
        $this->connect();
        $sql = '';
        // Lặp qua data
        ...
        $sql = 'UPDATE '.$table. ' SET '.trim($sql, ',').' WHERE '.$colunm.' = '.$where;
      return mysqli_query($this->__conn, $sql);
    }
     
    // Hàm delete
    function remove($table, $colunm, $where){
        $this->connect();
 
        // Delete
        $sql = "DELETE FROM $table WHERE $colunm =  $where";
        return mysqli_query($this->__conn, $sql);
    }
    ...
    function get_row($table,$colunm, $where){
        // Kết nối
        $this->connect();
        $sql = "SELECT * FROM $table WHERE $colunm =  $where";
        $result = mysqli_query($this->__conn, $sql);
